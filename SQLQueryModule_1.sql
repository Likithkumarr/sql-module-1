...
--WHERE Employee_ID = 5 -- Filter on one single integer value
--WHERE First_Name = 'likith' -- Filter on one single text value
--WHERE Employee_ID IN (1,2,5) -- Filter multiple values
--WHERE First_Name IN ('likith','charan')
--WHERE Employee_ID >= 3
--WHERE Employee_ID <= 3
--WHERE Employee_ID != 3 -- != and <> are both "not equal to" operators
--WHERE Employee_ID <> 3
--WHERE Employee_ID > 3 AND Address = 'India' -- BOTH conditions should be TRUE
--WHERE Employee_ID > 5 OR Address = 'India' -- Returns the record if atleast one condition is TRUE
--WHERE  Employee_ID NOT IN (1,5,7)
WHERE Employee_ID BETWEEN 3 AND 6 -- Returns all the records 3 and 6 and everything between

-- DISTINCT
	-- Helps us in retrieving only the distinct values from a table
SELECT DISTINCT Address ,first_name-- First_Name, Last_Name
FROM Employee
...
--WHERE First_Name LIKE 'l%' -- Filters out all the records which begins with d
--WHERE First_Name LIKE '%i%' -- Filter out all records which contains o in it
